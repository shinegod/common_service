...
public class AesCryptUtil {
    ...
    public static void main(String[] args) {
        ...
        assert decrypt(encrypt(plainDataInEn, password), password).equals(plainDataInEn)
                && decrypt(encrypt(plainDataInEn, emptyPassword), emptyPassword).equals(plainDataInEn);
        assert decrypt(encrypt(plainDataInCh, password), password).equals(plainDataInCh)
                && decrypt(encrypt(plainDataInCh, emptyPassword), emptyPassword).equals(plainDataInCh);
        assert decrypt(encrypt(plainDataInMixLn, password), password).equals(plainDataInMixLn)
                && decrypt(encrypt(plainDataInMixLn, emptyPassword), emptyPassword).equals(plainDataInMixLn);
        assert decrypt(encrypt(emptyPlainData, password), password).equals(emptyPlainData)
                && decrypt(encrypt(emptyPlainData, emptyPassword), emptyPassword).equals(emptyPlainData);
        assert decryptWithoutConfound(encryptWithoutConfound(plainDataInEn, password), password).equals(plainDataInEn)
                && decryptWithoutConfound(encryptWithoutConfound(plainDataInEn, emptyPassword), emptyPassword).equals(plainDataInEn);
        assert decryptWithoutConfoundWithCaseInsensitive(encryptWithoutConfoundWithCaseInsensitive(plainDataInCh, password), password).equals(plainDataInCh)
                && decryptWithoutConfoundWithCaseInsensitive(encryptWithoutConfoundWithCaseInsensitive(plainDataInCh, emptyPassword), emptyPassword).equals(plainDataInCh);
        assert decryptWithCaseInsensitive(encryptWithCaseInsensitive(emptyPlainData, password), password).equals(emptyPlainData)
                && decryptWithCaseInsensitive(encryptWithCaseInsensitive(emptyPlainData, emptyPassword), emptyPassword).equals(emptyPlainData);
    ...
    public static String encryptWithCaseInsensitive(String plainData, String password) {
        return encrypt(plainData, password, true, DEFAULT_MAX_CONFOUND_SIZE);
    }
