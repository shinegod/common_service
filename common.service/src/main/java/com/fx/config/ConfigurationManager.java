...
import com.fx.config.constants.ConfigConstant;
import com.fx.config.impl.AbstractConfiguration;
import com.fx.config.impl.ContextConfigurationWrapper;
import com.fx.config.impl.JarPropertiesConfiguration;
import com.fx.config.impl.MapConfiguration;
import com.fx.config.impl.PropertiesConfiguration;
import com.fx.config.loader.ConfigurationResourceFilter;
import com.fx.config.loader.PropertiesLoader;
import com.fx.config.loader.impl.ClassesPropertiesLoader;
import com.fx.config.loader.impl.JarPropertiesLoader;
...
public final class ConfigurationManager extends AbstractConfiguration {
    ...
    private String fillFileName(String fName) {
        if (fName != null && fName.endsWith(ConfigConstant.CONFIG_SUFFIX)) {
            return fName;
        } else {
            return fName + "_" + languageName + "_" + envName + ConfigConstant.CONFIG_SUFFIX;
        }
    }
    ...
    private void initEnvName() {
        String configEnvName = this.classesConfiguration.getString(ConfigConstant.APP_KEY_ENV);
        if (configEnvName != null && !"".equals(configEnvName.trim())) {
            ...
            LOGGER.warn("not config environment, use default value:{}.", ConfigConstant.APP_DEFAULT_ENV);
            this.envName = ConfigConstant.APP_DEFAULT_ENV;
        }
    }

    private void initLanguageName() {
        String language = this.classesConfiguration.getString(ConfigConstant.APP_KEY_LAN);
        if (language != null && !"".equals(language.trim())) {
            LOGGER.info("set config language, use config language:{}.", language);
            this.languageName = language.toLowerCase();
        } else {
            LOGGER.warn("not config language, use default value:{}.", ConfigConstant.APP_DEFAULT_LAN);
            this.languageName = ConfigConstant.APP_DEFAULT_LAN;
        }
    }
    ...
    private class JarResourceFilter implements ConfigurationResourceFilter<JarEntry> {
        ...
        private boolean checkConfigDir(JarEntry jarEntry) {
            ...
            return lowerEntryPath.endsWith(("_" + languageName + "_" + envName + ConfigConstant.CONFIG_SUFFIX).toLowerCase())
        ...
        private boolean checkRoot(JarEntry jarEntry) {
            String lowerEntryPath = jarEntry.getName().toLowerCase();
            return lowerEntryPath.startsWith(ConfigConstant.CONFIG_PREFIX)
                    && lowerEntryPath.endsWith(ConfigConstant.CONFIG_SUFFIX)
    ...
    private static class ClassResourceFilter implements ConfigurationResourceFilter<File> {
        ...
        public boolean checkResource(File resource) {
            return resource.getName().startsWith(ConfigConstant.CONFIG_PREFIX)
                    && resource.getName().endsWith(ConfigConstant.CONFIG_SUFFIX);
        }
    }
}
