...
/**
 * Created by bei2love@gmail.com on 15/5/14.
 */
public class ReloadableResourceBundleMessageSource extends AbstractMessageSource
    ...
    public void setBaseDirectory(String baseDirectory) {
        List<String> list = settingBaseDirectory(baseDirectory);
        if (list != null) {
            this.basenames = new String[list.size()];
            for (int i = 0; i < list.size(); i++) {
                String basename = "classpath:/"+list.get(i);
                Assert.hasText(basename, "Basename must not be empty");
                this.basenames[i] = basename.trim();
            }
        } else {
    ...
    private List<String> settingBaseDirectory(String baseDirectory) {
        // 如果路径中有空格则转换成%20,需先调用toURI()
        File file = null;
        try {
            file = new File(this.getClass().getResource("/").toURI().getPath()
                    + File.separatorChar + baseDirectory + File.separatorChar);
        ...
        if (file.isDirectory()) {
            ...
            for (File f : files) {
                if (f.isFile()) {
                    String baseName = baseDirectory
                            + "/"
                            + f.getName().substring(0,
                            f.getName().indexOf("_", 0));
