...
import com.fx.crm.online.UserOnlineService;
...
//        }
    ...
    public void validateSessions() {
        ...
        Pagination pageRequest = new Pagination();
        pageRequest.setLimit(100);
        List<UserCacheEntity<?>> page = userOnlineService.findExpiredUserOnlineList(expiredDate, pageRequest);

        //改成批量过期删除
//        while (!page.isEmpty()) {
            List<String> needOfflineIdList = Lists.newArrayList();
            for (UserCacheEntity<?> userOnline : page) {
                try {
                    ...
                    invalidCount++;
                    needOfflineIdList.add("" + userOnline.getUserId());
                }
